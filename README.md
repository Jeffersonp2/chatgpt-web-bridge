...
Nessa página, cada endpoint tem um exemplo ao lado do botão de teste. Os botões chamam as rotas diretamente e mostram o status HTTP e a resposta; a captura PNG do navegador aparece na própria página. Informe `LOCAL_API_KEY` quando configurada; para chat e áudio, escolha um arquivo ou digite uma mensagem. O dashboard não grava a chave; ela permanece no campo até fechar ou recarregar a página.
